...
<div id="modal-popup-box" class="modal">
    ...
    <form id="modal-popup-form" action="{{ route("admin.produtos.add") }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            <label>Nome:</label>
            <input class="{{ ($errors->first('nome') ? "input-error" : "") }}" type="text" name="nome" value="{{ $produto->nome }}" required disabled>
            <p class="error-msg">{{ $errors->first('nome') ? $errors->first('nome') : "" }}</p>
        </div>

        <div>
            <label>Descrição:</label>
            <textarea class="{{ ($errors->first('descricao') ? "input-error" : "") }}" name="descricao" rows="3" required disabled>{{ $produto->descricao }}</textarea>
        ...
        <div>
            <label>Preço:</label>
            <input class="nospinbox {{ ($errors->first('valor') ? "input-error" : "") }}" type="number" name="valor" value="{{ $produto->valor }}" min="0" step="0.01" required disabled>
            <p class="error-msg">{{ $errors->first('valor') ? $errors->first('valor') : "" }}</p>
        </div>

        <div>
            <label>Quantidade:</label>
            <input class="nospinbox {{ ($errors->first('quantidade') ? "input-error" : "") }}" type="number" name="quantidade" value="{{ $produto->quantidade }}" min="0" step="1" required disabled>
            <p class="error-msg">{{ $errors->first('quantidade') ? $errors->first('quantidade') : "" }}</p>
        </div>

        <div>
            <label>Categoria:</label><br>
            <textarea class="{{ ($errors->first('descricao') ? "input-error" : "") }}" name="descricao" rows="3" required disabled>{{ $produto->categoria->nome }}</textarea>
